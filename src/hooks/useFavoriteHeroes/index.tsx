...
const FavoriteHeroesProvider = ({ children }: FavoriteHeroesProviderProps) => {
  ...
  const getFavoriteHeroes = useCallback((): HeroData[] => {
    const storageHeroes = getStorageItem(storageKey)
    return storageHeroes || []
  }, [])

  const addFavoriteHero = useCallback((heroData: HeroData) => {
    const favoriteHeroes = getFavoriteHeroes()

    setStorageItem(storageKey, [...favoriteHeroes, heroData])
  }, [getFavoriteHeroes])

  const removeFavoriteHero = useCallback((id: number) => {
    const favoriteHeroes = getFavoriteHeroes()
    const newFavoriteHeroes = favoriteHeroes.filter(hero => hero.heroId !== id)

    setStorageItem(storageKey, newFavoriteHeroes)
  }, [getFavoriteHeroes])

  const isFavoriteHero = useCallback((id: number) => {
    const favoriteHeroes = getFavoriteHeroes()
    const isFavorite = !!favoriteHeroes.find(hero => Number(hero.heroId) === id)
    return isFavorite
  }, [getFavoriteHeroes])
  return (
...
const useFavoriteHeroes = (): FavoriteHeroesContextData => useContext(FavoriteHeroesContext)
